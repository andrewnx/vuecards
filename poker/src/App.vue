...
export default {
  ...
  methods: {
    ...
    dealFlop() {
      this.dealflopbutton = 0;
      this.mid.push(this.cards.shift());
      this.mid.push(this.cards.shift());
      this.mid.push(this.cards.shift());
      this.dealturnbutton = 1;

    },

    dealTurn() {
      this.dealturnbutton = 0;
      this.mid.push(this.cards.shift());
      this.dealriverbutton = 1;
    },

    dealRiver() {
      this.dealriverbutton = 0;
      this.mid.push(this.cards.shift());
      this.showdownbutton = 1;
    },
    showdown() {
      this.showdownbutton = 0;
      if (this.p1score > this.p2score) {
        this.p1score = "Player 1 Wins!";
      } else if (this.p1score < this.p2score) {
        this.p1score = "Player 2 Wins!";
      } else {
        this.p1score = "Players Split";
      }
    },
    ...
    restart() {
      ...
      while (this.mid.length > 0) {
        this.cards.push(this.mid.shift());
      }
      this.p1score = 0;
      this.p2score = 0;
...
<template>
  ...
  <button v-if="dealflopbutton" @click="dealFlop">Deal flop</button>
  <button v-if="dealturnbutton" @click="dealTurn">Deal turn</button>
  <button v-if="dealriverbutton" @click="dealRiver">Deal river</button>
  <button v-if="showdownbutton" @click="showdown">Showdown</button>
  ...
  <div id="mid">
    ...
    <transition-group name="p1cards" appear>
      <div v-for="card in mid" :key="card.id" class="player" :class=suitColor[card.suit]>
        <Card :card-rank="card.rank" :card-suit="card.suit" />
      </div>
    </transition-group>
  </div>
